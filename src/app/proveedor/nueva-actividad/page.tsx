...
import { crearActividad } from '@/lib/store';
...
export default function NuevaActividadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fechaAct = new Date(fechaHoraISO);
    if (fechaAct <= new Date()) {
      setError('La fecha y hora deben ser futuras.'); setEnviando(false); return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al publicar la actividad.');
    } finally {
      setEnviando(false);
    }
  };
  ...
  if (exito) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center">
        ...
        <h2 className="font-display text-4xl text-ink mb-3">¡Actividad publicada!</h2>
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-10 py-12">
      ...
      <div className="mb-10">
        <h1 className="font-display text-4xl lg:text-5xl text-ink mb-2">Publicar actividad</h1>
        <p className="text-coffee/80">Completa los datos y tu actividad quedará disponible al instante.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-cream rounded-soft border border-coffee/10 p-8 lg:p-10 space-y-8 shadow-soft">
        ...
        <div className="grid sm:grid-cols-4 gap-5">
          ...
          <div>
            ...
            <p className="text-xs text-coffee/60 mt-1">Mínimo 55 (R-1)</p>
          </div>
          <div>
        ...
        <div>
          ...
          <p className="text-xs text-coffee/60 mt-1">
            Si la dejas vacía usaremos una imagen predeterminada.
          </p>
        </div>
        ...
        <div className="pt-2 bg-sand/30 -mx-8 lg:-mx-10 px-8 lg:px-10 py-5 border-t border-coffee/10 rounded-b-soft">
          <p className="text-sm text-coffee/60 mb-4">
            Puedes crear actividades gratuitas o de pago. El sistema simula el proceso de cobro.
          </p>
          ...
          >
            {enviando
              ? 'Publicando...'
              : <><Save className="w-5 h-5" /> Publicar actividad</>
            }
          </button>
        </div>
      </form>
